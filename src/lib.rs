...
pub fn sign<C: bitcoin::secp256k1::Signing + bitcoin::secp256k1::Verification>(secret_key: &SecretKey, secret_nonce: &SecretNonce, aggregate_nonce: &PublicNonce, keys: &SignerPublicKeys, message: &[u8], secp_context: &Secp256k1<C>) -> PartialSignature {
	...
	let is_valid = verify_partial_signature(&s, &secret_nonce.to_public(&secp_context), aggregate_nonce, &secret_key.public_key(&secp_context), keys, message, secp_context);
	assert!(is_valid);
	s
}
...
pub fn verify_partial_signature<C: bitcoin::secp256k1::Verification + bitcoin::secp256k1::Signing>(partial_signature: &SecretKey, partial_public_nonce: &PublicNonce, aggregate_public_nonce: &PublicNonce, partial_public_key: &PublicKey, keys: &SignerPublicKeys, message: &[u8], secp_context: &Secp256k1<C>) -> bool {
	let ordered_keys = keys.ordered_keys();
...
fn verify_partial_signature_with_ordered_keys<K: Borrow<PublicKey>, C: bitcoin::secp256k1::Verification + bitcoin::secp256k1::Signing>(partial_signature: &SecretKey, partial_public_nonce: &PublicNonce, aggregate_public_nonce: &PublicNonce, partial_public_key: &PublicKey, ordered_keys: &[K], message: &[u8], secp_context: &Secp256k1<C>) -> bool {
	let aggregate_public_key = aggregate_ordered_keys(ordered_keys, &secp_context);
	...
	let partial_signature_point = partial_signature.public_key(&secp_context);
...
mod tests {
	...
	fn test_signature_aggregation() {
		...
		}
		{
			...
			println!("aggregate pubkey (Q, reversed order): {}", hex::encode(aggregate_public_key.0.serialize()));
