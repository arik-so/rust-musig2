...
pub(super) fn sort_keys<K: Borrow<PublicKey>>(keys: &[K]) -> Vec<PublicKey> {
...
pub(super) fn key_aggregation_coefficient_internal<K: Borrow<PublicKey>>(sorted_keys: &[K], public_key: &PublicKey) -> Scalar {
	// could be memoized
...
mod tests {
	...
	use crate::key_aggregation_coefficient_internal;
	use crate::key_preparation::{aggregate_keys, sort_keys};
